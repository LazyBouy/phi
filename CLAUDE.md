...
5. **Retrospective review.** Read retrospector's draft; propose standards updates to user; apply approved updates with version bumps logged in `.claude/agents/_changelog.md`. **Final step (added 2026-05-09 per CH-17 retro Row 1, USER REQUESTED, cycle hex `40c4d759`; placement corrected post-CH-17 close): after standards updates landed + cycle-index row flipped to `retro-complete`, run `/root/rust-env/cargo/bin/cargo clean --manifest-path /root/projects/phi/baby-phi/Cargo.toml` to reclaim disk before user commit.** Capture `du -sh /root/projects/phi/baby-phi/target` BEFORE + `df -h /root | head -3` AFTER and log disk reclaimed in the cycle-audit's §7 metrics row. Rationale: CH-17 hit a mid-cycle Bash sandbox failure when compiled test binaries consumed >160 GB; cleanup at gate-5 close ensures next chunk opens with a clean slate. **Why gate-5 close (not chunk-implementer chunk-seal)**: gate-4 MUST-RUN (clippy + cargo test) and the retrospector's permissions-audit scripts BOTH rebuild target/ — running cargo-clean any earlier wastes the cleanup. CH-17 reclaimed 107 GB at chunk-seal but rebuilds to 67 GB by gate-5 close; orchestrator-applied final cargo-clean reclaimed another 68.7 GB to truly close at 154 GB free.
