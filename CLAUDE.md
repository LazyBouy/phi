...
4. **Final cycle re-audit (mandatory).** After all sub-agent audits go green, I personally re-read every diff, re-run full workspace tests + 4 CI guards, run phi-core-leverage-check + k8s-readiness-check skills, verify all paperwork. Write `cycle-audit.md`. May re-trigger Implementer or Planner re-spawn. Never skipped. **Live disposition-asserting close-gate — REQUIRED (NEW 2026-08-18 per joint-retro `06f9e112..53c7780f` #1), NOT merely deferred-but-owned, whenever the chunk ships a per-agent / model-facing / storage-isolation deliverable**: render + read the actual PRODUCT artifact where the fix MANIFESTS (the model wire for tool-catalog / permission / model-facing changes; the filesystem / emitted event / DB row for storage-isolation) per `[[feedback_render_transcript_close_gate]]` Rules 4/6/7/8, and assert the DISPOSITION against the intended contract (no-error is necessary but NOT sufficient). Use the `live-close-gate` skill as the driver. Evidence: 2-of-3 close-gates this MA batch found + fixed real gaps the green deterministic suites were structurally blind to (MA-06 live-overlay non-application; MA-07 table-order reorder); MA-08's clean gate still proved the load-bearing file≡inline parity on the wire (2197==2197 tokens). **MUST-RUN list (sub-agents cannot execute these reliably) — PROJECT-CONDITIONAL as of Phase 1.5 2026-05-28:**
