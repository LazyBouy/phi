...
- **§B Hot allow-rule candidates** — input_signatures with `PermissionRequest` count ≥ 3 AND no allow-rule match. Propose a rule pattern derived from the signature (see "Proposed rule synthesis" below).
  ...
  - **Cross-cycle-bleed attribution methodology** (added v4.2 per CH-27 retro Row 8, cycle hex `0edcaba9`): when the cycle window captures BOTH CH-NN direct work AND parallel-cycle work in the shared `.claude/tool-use.log` telemetry stream, each ≥3-prompt cluster row in §B MUST classify as either **"CH-NN-direct"** OR **"cross-cycle-bleed (cite parallel cycle hex via sample-input-anchor)"** — never aggregated without attribution. Use **sample-input introspection** to discriminate: if the cluster's representative sample input cites artifacts from a different cycle's workspace (e.g., `/tmp/audit-chXY*.jsonl`, `/tmp/chXY-planner-output.md`, edits to `/root/projects/phi/i-phi/...` from a baby-phi-context cycle, etc.), classify as cross-cycle-bleed with the parallel cycle hex cited. **Subtract cross-cycle-bleed clusters from the CH-NN actionable Hot-candidate count** in §G trend reporting; otherwise parallel-cycle clusters mask CH-NN-direct candidates and inflate apparent prompt counts. Rationale: CH-27 telemetry captured BOTH CH-27 gate-3+4 work AND parallel CH-03-i-phi retrospector-cycle work (commit `1af57e4` at 14:40Z, 10 min after CH-27's cycle-audit close); **51 of 98 PermissionRequest prompts (52%) were CH-03-i-phi parallel-cycle bleed** (sample inputs cited `/tmp/audit-ch02c.jsonl` and edits to `/root/projects/phi/.claude/agents/chunk-planner.md` from i-phi retro), NOT CH-27 direct workflow noise. Without attribution, CH-27's apparent actionable hot candidate count was 4-5; with attribution, only 1 actionable. The shared-telemetry-stream + parallel-cycle reality is now a structural feature of the workflow; surfacing the attribution prevents trend-table inflation + spurious settings.json rule proposals.
...
---
*Generated by `.claude/skills/permissions-audit/SKILL.md` v1 from `.claude/tool-use.log`.*
```
