...
5. **Run cargo at phase boundary** with `-j 4` cap (memory `feedback_cargo_jobs_cap.md`):
   ...
   **`clippy::redundant_locals` trap on `async move` captures (added v27 per joint-retro `4e4d7547..7cdc82fa` proposal #5, i-phi v0.5; CH-V05-03 D-2)**: a `Copy`-typed value (`Duration`, `u64`, `bool`, an enum that is `Copy`, …) captured into an `async move` block does NOT need a `let x = x;` rebind — the block copies it directly, and the rebind trips `clippy::redundant_locals` which FAILS under `-Dwarnings`. Only **non-`Copy`** captures (`Arc<_>`, `String`, `Vec<_>`) need the explicit clone-into-local before the `async move` (to keep an outer `Fn` closure callable). Rule of thumb: clone `Arc`/`String` into a local; let `Copy` values fall through.
