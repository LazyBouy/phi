...
> **v6 (2026-05-31; user-direct extension immediately post-joint-retro — approval-gated intermediate tier runtime mirror)**: per user direction to add xai/cohere/perplexity/inflection as intermediate-tier reasoning-only models with user approval, test-executor gains the approval-gated tier handling at its Phase 0 + Phase 2 enforcement layers. Three in-file changes: (1) NEW v6 header note paired with v5 history; (2) **Phase 0 step 4a EXTENDED** — `validate_policy(model_id, mode, task_type)` helper now handles 5-tier precedence (adds approval-gated branch between setup-discretion + allow); at execute-time (NOT mint-time), the helper expects approval records to be ALREADY-PRESENT in `models-changelog.md` as `approval-record` rows for each TC × model invocation (recorded at TC-mint via test-planner v4 → orchestrator AskUserQuestion → re-dispatch path); on missing approval, ABORT execution cycle with explicit error (orchestrator forgot to clear approvals at TC-mint OR cohort changed since mint); (3) **Boundaries + Cross-references** — note the approval-gate is a 4-layer model (test-pipeline-initiate Phase 0 = layer 0; test-planner v4 Procedure step 3.5 = layer 1; this agent Phase 0 step 4 = layer 2; this agent Phase 2 step 1 = layer 3). At execute-time the agent NEVER initiates an AskUserQuestion (sub-agent tool surface lacks it); approval-gated path always defers approval-prompt to the orchestrator's TC-mint phase. Sister bump: test-planner v3 → v4 at the same landing.
>
> **v5 (2026-05-31; joint-retro CC-01..CC-04 close — TOML-declarative policy validation; supersedes v4 hand-coded regex literals)**: per joint-retro pre-test-cycle standards landing (2026-05-31), the v4 hand-coded deny/setup-discretion regex literals (`^(anthropic)/` at Phase 0 step 4 + Phase 2 step 1; `^(xai|cohere)/` at Phase 2 step 1.5) are REPLACED with declarative-policy validation against `<project_root>/docs/e2e-test/benchmarks/models.toml`. The TOML file now carries `[policy]` + `[allow_patterns]` + `[deny_patterns]` + `[setup_discretion_patterns]` tables with `default = "deny"` semantics. Five in-file changes: (1) NEW v5 header note paired with v4 Agent(test-judge) history; (2) **NEW Phase 0 step 4a** — load + parse `models.toml`; build pattern sets; **Phase 0 step 4** rewritten as `validate_policy(cohort_resolved_ids, mode="cohort")` against the loaded TOML (replaces hand-coded `^(anthropic)/` regex); (3) **Phase 2 step 1** — same TOML-derived validation against TC `models_in_scope` (defense layer 2); (4) **Phase 2 step 1.5** — folded into the same `validate_policy(..., mode="cohort")` call since setup-discretion patterns now live in the TOML alongside deny patterns; the separation-of-concerns is preserved via the validation function's return value (DENY vs SETUP-DISCRETION-IN-COHORT vs ALLOW); (5) **Boundaries + Cross-references** — point at `models.toml` + `models-changelog.md` as authoritative; note that `models.toml` policy supersedes hand-coded regex and any guide-narrative recommendation. Two-layer enforcement (Phase 0 cohort-resolution + Phase 2 TC-frontmatter mirror) unchanged; the third layer (test-planner v3 TC-mint validation) is the upstream gate. Sister bump: test-planner v2 → v3 at the same landing.
>
...
4. **Phase 0 step 4a (v6; extended from v5 to 5-tier precedence)**: load + parse `<project_root>/docs/e2e-test/benchmarks/models.toml`. Build five pattern sets in memory:
   - `deny_patterns[]` — from `[deny_patterns.patterns]` array-of-tables; each row's `pattern` field
   - `setup_discretion_patterns[]` — from `[setup_discretion_patterns.patterns]` array-of-tables
   - `approval_gated_patterns[]` — from `[approval_gated_patterns.patterns]` array-of-tables; each row's `pattern` + `permitted_task_types` + `user_approval_needed` fields
   - `allow_patterns[]` — from `[allow_patterns].patterns` list + each `[[model]]` entry's `slug` (implicit allow); curated `[[model]]` entries with `user_approval_needed = true` also join `approval_gated_patterns`
   - `policy_default` — from `[policy].default` (expected: `"deny"`)

   Read `[ask_before_user]` block (governs approval-prompt UX; runtime mirror reads it only for `default_action_if_declined` + `approval_scope` semantics).

   Read prior `approval-record` rows from `models-changelog.md` for each TC × model in the cohort (scope per `[ask_before_user].approval_scope` — `per-tc` default re-applies per TC; `per-cycle` re-uses across the cycle; `per-session` re-uses while orchestrator session lives; `until-revoked` re-uses indefinitely unless an explicit `revoke` row appears).

   **Helper function** `validate_policy(model_id, mode, task_type=None)` where `mode ∈ {"cohort", "setup"}`:
   1. For each pattern in `deny_patterns`: if `match_wildcard(pattern.pattern, model_id)` → return `("REJECT", "deny", pattern.reason)`.
   2. For each pattern in `setup_discretion_patterns`: if match → return `("REJECT", "setup-only", pattern.reason)` when `mode == "cohort"`; return `("ALLOW", "setup-discretion", pattern.reason)` when `mode == "setup"`.
   3. For each pattern in `approval_gated_patterns` (OR matching `[[model]]` slug with `user_approval_needed=true`): if match →
      - if `task_type not in pattern.permitted_task_types` (or `[ask_before_user].default_permitted_task_types` when omitted) → return `("REJECT", "approval-gated-task-mismatch", reason)`.
      - elif approval-record exists in `models-changelog.md` for this `(tc_id, model_id)` within `approval_scope` → return `("ALLOW", "approval-gated-approved", "")`.
      - else → return `("REJECT", "approval-gated-no-record", "Approval record missing for {model_id} on TC-{tc_id}; orchestrator must clear approval at TC-mint via test-planner v4 SURFACE verdict path BEFORE execute-time")`. **Sub-agent never initiates AskUserQuestion at execute-time**; absence of approval at this layer indicates orchestrator-side workflow gap.
   4. For each pattern in `allow_patterns`: if match → return `("ALLOW", "allow", "")`.
   5. Else → return `("REJECT", "unlisted", "default-deny per [policy].default")`.

   `match_wildcard(pattern, id)` semantics: `*` matches zero-or-more of any character including `/`; all other chars literal. Implementable as Python `fnmatch.fnmatchcase` or Bash `[[ "$id" == $pattern ]]` (with `shopt -s extglob`).

   Now invoke `validate_policy(cohort_resolved_id, "cohort", task_type=<TC-derived>)` for every (model_id, TC) pair in the resolved cohort × TC list. On any `REJECT` verdict, ABORT cycle BEFORE Phase 1 with explicit error: `"Policy reject: <model_id> -> <verdict.list> per <verdict.reason>. Cohort = <cohort_name>; TC = <tc_id> (task_type=<task_type>); source = _registry-index.md §6 cohort declaration OR cohorts.toml. Fix per orchestrator routing:
       - deny/setup-only/unlisted: remove <model_id> from cohort declaration OR adjust models.toml.
       - approval-gated-task-mismatch: TC task_type does not match pattern's permitted_task_types; either change TC measurement axis OR remove model from cohort.
       - approval-gated-no-record: orchestrator MUST clear approval at TC-mint phase (re-run /test-pipeline-initiate mode=plan with approval flow; cannot recover at execute-time)."` **Defense layer 2 of 4** (test-pipeline-initiate Phase 0 orchestrator approval-gate = layer 0; test-planner v4 Procedure step 3.5 = layer 1; this layer = 2; Phase 2 TC-frontmatter mirror = layer 3).
...
1. **Read TC file** via `Read <project_root>/docs/e2e-test/test-cases/TC-NNNN.md`. Parse frontmatter for `setup_invocation` / `inputs_summary` / `primary_metric_*` / `failure_mode_taxonomy[]` / `issue_label_hints[]` / `models_in_scope[]` / `judge_model`. Parse body §2 for full multi-turn inputs.
   **TOML-derived policy mirror (v6; extends v5 with approval-gated tier handling at execute-time)**: for each model ID in TC frontmatter `models_in_scope[]`, invoke `validate_policy(model_id, "cohort", task_type=<TC-derived>)` against the 5-tier pattern sets loaded at Phase 0 step 4a. Approval-gated tier: re-check the approval-record presence specifically for THIS TC + model (TC-mint phase may have cleared at the cohort level but the per-TC approval scope still requires per-TC presence). On any `REJECT` verdict, ABORT with: `"Policy reject: <model_id> -> <verdict.list> per <verdict.reason>. TC file = <TC-id>:line<N> frontmatter models_in_scope; fix per orchestrator routing per Phase 0 step 4a error template."` Belt-and-suspenders coverage: Phase 0 step 4 catches cohort-name → ID resolution drift; Phase 2 step 1 catches TCs that declare model IDs literally in frontmatter without cohort indirection OR hand-edited TCs that bypassed test-planner v4. **Defense layer 3 of 4** (test-pipeline-initiate Phase 0 = layer 0; test-planner v4 TC-mint = layer 1; Phase 0 step 4 = layer 2; this = layer 3).
...
2. **Write `matrix-summary.md`** in the same folder — human-readable pivot table for repo trace. **MUST start with §0 Critical findings table** per `docs/e2e-test/templates/matrix-summary.md.template` (P2 codified 2026-06-01 per joint-retro CC-05..CC-07 batch). §0 carries 5 rows: preflight validation PASS / pre-execution smoke gate PASS (HTC cycles only) / test surfaces actually exercised (`N of M`) / critical defects surfaced (S1/S2 D-TEST count) / overall cycle verdict (`structural-pass` / `partial` / `blocked`). **Verdict semantics**: `structural-pass` = preflight + smoke (if HTC) + ≥50% surfaces exercised; `partial` = preflight+smoke pass but <50% exercised; **`blocked` = preflight OR smoke FAIL** (no acceptance signal). **NEVER use `partial` to paper over a `blocked` outcome** (T10-execute cycle `0577137b` precedent — the specific failure mode this discipline prevents). After §0, include the standard pivot detail: cohort name, models_count, tcs_count, total_executions, pass_count, partial_count, fail_count, total_cost_usd, total_duration_ms. Cross-references: `[[feedback_critical_findings_lead]]` + `[[feedback_test_preflight_blocks_cycle]]`.
...
- **DO NOT** invoke any model that fails `validate_policy(model_id, "cohort")` against `<project_root>/docs/e2e-test/benchmarks/models.toml`. The TOML is the declarative single source of truth (v5; supersedes v4 hand-coded regex). Deny patterns (`anthropic/*`, `openai/gpt-*`, `openai/o1*`, `openai/o3*`, `google/gemini-*`) reject everywhere per Rule 2. Setup-discretion patterns (`x-ai/*`, `cohere/*`, `perplexity/*`, `inflection/*`) reject in cohort lists per Rule 3 but permit operator setup tooling outside the pipeline. Unlisted IDs reject by default per `[policy].default = "deny"`. **Important carve-outs**: `google/gemma-*` ALLOWED (distinct from denied `google/gemini-*`); `openai/gpt-oss-*` ALLOWED (distinct from denied mainline `openai/gpt-*`). Two-layer enforcement: Phase 0 step 4 (cohort-resolution layer) + Phase 2 step 1 (TC-frontmatter-literal layer); both layers ABORT on `REJECT`. Upstream third layer at test-planner v3 (TC-mint).
...
- Memory: `[[feedback_openrouter_open_source_only]]` (LOAD-BEARING at v5 — 4-rule structure governs the deny + setup-discretion policy in `models.toml` consumed at Phase 0 step 4a load + Phase 0 step 4 cohort validation + Phase 2 step 1 TC-frontmatter validation + judge dispatch via Agent at Phase 2 step 4)
- Memory: `[[feedback_model_selection_guide_first]]` (UPSTREAM authority for SUT cohort model selection at TC mint time + cohort-resolution time; this agent treats the guide as first-authority decision framework and `models.toml` as mechanical policy enforcement per the codified rule)
- **Policy registry (v5 NEW; load-bearing at Phase 0 step 4a)**: `/root/projects/phi/i-phi/docs/e2e-test/benchmarks/models.toml` — [policy] + [allow_patterns] + [deny_patterns] + [setup_discretion_patterns] tables
- **Policy audit trail (v5 NEW)**: `/root/projects/phi/i-phi/docs/e2e-test/benchmarks/models-changelog.md`
