...
4. **NEW Phase 0 step 4a (v5; replaces v4 hand-coded regex)**: load + parse `<project_root>/docs/e2e-test/benchmarks/models.toml`. Build four pattern sets in memory:
   ...
   - `allow_patterns[]` — from `[allow_patterns].patterns` list + each `[[model]]` entry's `slug` (implicit allow)
   - `policy_default` — from `[policy].default` (expected: `"deny"`)

   **Helper function** `validate_policy(model_id, mode)` where `mode ∈ {"cohort", "setup"}`:
   ...
   3. For each pattern in `allow_patterns`: if match → return `("ALLOW", "allow", "")`.
   4. Else → return `("REJECT", "unlisted", "default-deny per [policy].default")`.
   ...
   Now invoke `validate_policy(cohort_resolved_id, "cohort")` for every model ID in the resolved cohort. On any `REJECT` verdict, ABORT cycle BEFORE Phase 1 with explicit error: `"Policy reject: <model_id> -> <verdict.list> per <verdict.reason>. Cohort = <cohort_name>; source = _registry-index.md §6 cohort declaration OR cohorts.toml. Fix: remove <model_id> from cohort declaration OR add an [allow_patterns].patterns entry to models.toml + append a row to models-changelog.md."` Defense layer 1 of 2 (TC-frontmatter-side check at Phase 2 step 1 is layer 2).
...
1. **Read TC file** via `Read <project_root>/docs/e2e-test/test-cases/TC-NNNN.md`. Parse frontmatter for `setup_invocation` / `inputs_summary` / `primary_metric_*` / `failure_mode_taxonomy[]` / `issue_label_hints[]` / `models_in_scope[]` / `judge_model`. Parse body §2 for full multi-turn inputs.
   **TOML-derived policy mirror (v5; supersedes v4 hand-coded regex literals)**: for each model ID in TC frontmatter `models_in_scope[]`, invoke `validate_policy(model_id, "cohort")` against the pattern sets loaded at Phase 0 step 4a. On any `REJECT` verdict, ABORT with: `"Policy reject: <model_id> -> <verdict.list> per <verdict.reason>. TC file = <TC-id>:line<N> frontmatter models_in_scope; fix the TC frontmatter OR update models.toml + models-changelog.md if the model should be allow-listed."` Belt-and-suspenders coverage: Phase 0 step 4 catches cohort-name → ID resolution drift; Phase 2 step 1 catches TCs that declare model IDs literally in frontmatter without cohort indirection. **Defense layer 2 of 2** (test-planner v3 TC-mint validation is the upstream gate; this is the runtime safety net).
