...
10a. **Author `## §1 — Locked fork details` populated bodies (v32 P-plan-1-v32; mandatory at iter-1)** — for each fork in `## Forks for orchestrator`, write the corresponding `#### F<N> = F<N>.<planner-rec-letter>` subsection in §1 with 3-sentence Code-level binding + 3-sentence Rationale + 3-sentence Defers blocks per the per-chunk-planning-template `## §1 — Locked fork details` structure. Mark each subsection with `*(pre-lock draft; finalizes at gate-1)*` italic tag. **End-of-draft self-check**: invoke skill `chunk-template-validate-locked-appendix` against the draft path; if it returns FAIL, re-emit §1 + re-run skill until PASS. Belt-and-suspenders: chunk-archive-plan v4 hard-assertion re-invokes the skill at archive-tier (both layers fire).
10b. **Bundle-framing for COUPLED forks (v36 P-plan-1-v36; joint-retro CC-22..CC-27 #1, 2026-06-12)** — when ≥ 2 forks are **coupled** (one fork's viability depends on how another locks — established by the P0, e.g. "F-X.a is only viable IF F-Y.a is locked"), do NOT present them as N independent fork votes (independent votes can produce an *incoherent* set — e.g. one fork relying on a default another fork didn't flip). Instead present them at gate-1.5 as **N coherent BUNDLES** in the `## Forks for orchestrator` section: each bundle is an internally-consistent lock-set the user can choose with ONE AskUserQuestion choice (a `### Bundle map (lock as a set)` table mapping each bundle → its per-fork option). Draft the §1 bodies for the **planner-rec bundle**, and draft the alternative bundle(s) as `alt` in the Forks section, so a USER-DIVERGENT bundle lock is a fast **narrow iter-2 re-author** of only the divergent fork subsections (v32 path). State the coupling explicitly in the affected §1 lock bodies (e.g. "this body is viable ONLY because F-Y.a is locked; if F-Y flips to .b, this fork flips to F-X.b"). **Canonical first application**: CC-27 (`546e6713`) — 2 bundles (ergonomic-default vs conservative-warn) from 6 coupled permission-UX forks; the user locked the non-rec bundle → clean iter-2 narrow re-author, 0 audit re-spawns.
10c. **Cross-crate (phi-core-editing) consumption/publish-path fork (v38 P-plan-1-v38; NEW 2026-09-10 per FI+KC joint-retro `48537f70` prop 3B / FI-28)** — when a chunk edits phi-core SOURCE (only when it is a genuine general kernel feature per `[[feedback_phi_core_kernel_minimal]]`, itself a user-gated chief fork), the plan MUST carry a MANDATORY fork on the **consumption / release path**. Read the consumer's `Cargo.toml` + `Cargo.lock` for the phi-core dep: does it build from **crates.io** (`Cargo.lock` `source = "registry+…"`) or the **local source tree** (a `path`/workspace dep)? **If crates.io**: a local kernel edit is **INERT to the consumer's build until PUBLISHED** — the plan MUST lock a publish sub-step (bump phi-core `Cargo.toml` version → `cargo publish --dry-run` → `cargo publish`; network + user-gated at the irreversible step; `CARGO_REGISTRY_TOKEN` from `.env`) + a `cargo update -p <crate>` in the consumer (a caret dep like `^0.15` auto-resolves). The cross-crate seal's parent gitlink bump then tracks BOTH submodules (phi-core + the consumer), never a third. Also lock the **byte-compat** requirement (the kernel default must reproduce the pre-edit behaviour — FI-28 `messages_url` default = the old `const API_URL`) and note that a P0 grounded before a prior sibling sealed needs RE-GROUNDING at the current HEAD. **This is a planner fork, not P0-luck**: at FI-28 the crates.io-consumption catch was the planner's (F1), not the P0's — a local kernel edit would otherwise have shipped INERT. Canonical: FI-28 (`f3a2bf69`, published phi-core 0.15.1, ADR-0087).
...
### v35 — Two-update bundle from i-phi v0.5 joint-retro `4e4d7547..7cdc82fa` (P-plan-1-v35 builder-threading call-site grounding + P-plan-2-v35 substrate-module LOC-cap calibration)

#### P-plan-1-v35 — Ground builder-threading call-site counts (LOW; closes CH-V05-03 D-3 — `~30` vs actual `15` estimate drift)

When a chunk threads a NEW field into an existing struct via a `with_*` builder (default-`None` additive pattern, e.g. `IpcState::with_log_capture` / `with_pending_decisions`), the plan §1/§6 narrative often cites how many existing `<Type>::new(...)` call-sites stay 0-edit. **Ground that count via `grep -rc '<Type>::new(' src/ tests/` at plan-draft time — do NOT estimate.** CH-V05-03 plan §1 F2.a said "~30" call-sites, the ADR said "14", actual was 15; harmless (the 2-arg-stability invariant was asserted correctly), but the estimate drift is exactly what a one-line grep eliminates. Pairs with the `baseline-snapshot` Axis-C discipline (this is the call-site analogue of struct-field grounding).

#### P-plan-2-v35 — Substrate-module LOC-cap calibration (LOW; CH-V05-01/02/03 substrate cohort)

A module that carries a `tracing_subscriber::Layer` impl OR a process-global accessor (`OnceLock`/`Arc<Mutex<…>>` registry) OR an inline-tracing-test block runs **~1.2–1.8× a plain-struct module** of the same field count. When predicting §3.B caps for substrate/registry modules (log-capture, hook-sink, pending-decision registry, metrics facade), apply the multiplier rather than the plain-struct baseline. Empirical: CH-V05-02 `logcapture.rs` 355 vs cap 220 (1.6×); CH-V05-03 `pending.rs` 240 vs cap 230 (1.04× — calibrated). Surface the multiplier in the §3.B per-file LOC-cap body. Extends v23 P-plan-1 / v24 P-plan-1-v24 functional-scope derivation.
...
## v32 additions (Chunk D intermediate-stabilization `36caa39f`, 2026-05-26)

### P-plan-1-v32 — Iter-1 §1 Locked fork details with planner-rec bodies pre-filled (HIGH; deprecates v22 P13 / v23 P-plan-3 ALWAYS-FIRE iter-2 re-spawn for planner-rec-clean cycles)

The planner MUST ship `## §1 — Locked fork details — [DRAFT planner-rec bodies; pre-lock]` as a **populated section at iter-1 plan-draft time** — not as a placeholder, not as a stub deferred to iter-2 re-spawn. Each fork in the chunk's `## Forks for orchestrator` section gets a `#### F<N> = F<N>.<planner-rec-letter>` subsection in §1 carrying the 3–6 sentence body covering:

1. **Code-level binding** (3 sentences) — what code site changes; what struct/trait/method/file:line absorbs the lock; how the locked option manifests in source-tree shape (NEW file vs EXTEND file vs trait-method addition vs config schema add).
2. **Rationale** (3 sentences) — why this planner-rec over the named alternatives in `## Forks for orchestrator`; cite the decisive tradeoff axis (user-visible delivery vs cycle scope vs precedent-alignment vs cascade footprint); reference any cross-cycle pattern that informed the recommendation.
3. **Defers (if chosen)** (3 sentences) — what features NOT shipping under this option; cite allocation chunk-IDs (e.g., `M6-DEFERRED-NN` / `CH-NN+`) or "none deferred" if the option ships the full surface for this chunk; note any downstream consumer that inherits the deferred state.

Each subsection MUST be marked clearly with `*(pre-lock draft; finalizes at gate-1 lock unless USER-DIVERGENT lock is selected)*` italic tag at the H4 header line.

**Conditional structure at gate-1**:

```
iter-1 §1 populated WITH planner-rec bodies (per P-plan-1-v32)
├─ ALL forks lock at planner-rec → skip iter-2; archive at iter-1 (gate-1.5 skip-condition per outer CLAUDE.md P-orch-8 + chunk-initiate Phase 1.5 Step A skip-condition)
└─ ≥ 1 fork USER-DIVERGENT → iter-2 re-spawn fires (per Step B material-scope-expansion); ONLY divergent F<N> subsections re-authored; planner-rec subsections preserved at iter-1 draft
```

**Self-check at end-of-draft**: invoke skill `chunk-template-validate-locked-appendix` against the draft (heading exists / subsection count ≥ fork count / each subsection ≥ 3 sentences). The skill returns PASS/FAIL; planner re-emits + re-runs until PASS. Belt-and-suspenders: chunk-archive-plan v4 hard-assertion re-invokes the same skill independently — both layers fire.

**Deprecation of v22 P13 / v23 P-plan-3 ALWAYS-FIRE iter-2 re-spawn for planner-rec-clean cycles**:

v22 P13 + v23 P-plan-3 ALWAYS-FIRE iter-2 re-spawn was the canonical pre-v32 mechanic for absorbing the locked-fork-details appendix post-gate-1. It correctly closed the 3-of-3-cycle CH-03 / CH-04 / CH-05 i-phi regression where the appendix went missing at iter-1 archive. With v32, the appendix is **pre-filled at iter-1** so iter-2 re-spawn is no longer required for planner-rec-clean cycles — the appendix is already present + populated; gate-1 just confirms the planner-rec selection.

**Hold-period (2-3 cycles AFTER v32 ships; spans BOTH projects)**:

- Hold-period CROSS-PROJECT: cycles count across baby-phi AND i-phi (e.g., i-phi CH-11b + baby-phi CH-NN both count toward the 2-3 cycle threshold).
- **During hold-period**: v23 P-plan-3 ALWAYS-FIRE iter-2 re-spawn remains available as a fallback. The chunk-archive-plan hard-assertion is the primary defense; if any cycle in the hold-period regresses (iter-1 §1 missing/malformed AND not caught by chunk-archive-plan hard-assertion), revert v32 + investigate.
- **After hold-period (3+ clean cycles)**: v23 P-plan-3 ALWAYS-FIRE for planner-rec-clean cycles is fully deprecated. USER-DIVERGENT iter-2 re-spawn unchanged (still mandates Step B material-scope-expansion absorption).

**USER-DIVERGENT path unchanged**:

When ≥ 1 fork locks differently from planner-rec, iter-2 re-spawn fires per chunk-initiate Phase 1.5 Step B material-scope-expansion (the existing mechanic). The ONLY change: at iter-2, the planner re-authors ONLY the F<N> subsections corresponding to USER-DIVERGENT locks. Planner-rec subsections preserve their iter-1 draft wording verbatim. iter-2 plan body sections §2-§13 (Context / Concept walk / Scope / Drifts / ADR / Carry-forward / Phase plan / Tests / Cargo / Close criteria / Audit envelope / Confidence) absorb the divergent-lock impact per chunk-planner v23 P-plan-1 + v24 P-plan-1-v24 functional-scope cap re-derivation + v32 P-plan-1-v32 (renamed below) + downstream rules unchanged.

**Cross-references**:

- per-chunk-planning-template.md (canonical for both projects) `## §1 — Locked fork details (per chunk-planner v32 iter-1 template; planner-rec bodies pre-filled)` — the template structure consumed at iter-1 draft.
- chunk-template-validate-locked-appendix skill — mechanical 4-step PASS/FAIL validation.
- chunk-archive-plan v4 — hard-assertion invokes the validation skill BEFORE minting cycle-folder + cycle-index row.
- chunk-initiate Phase 1.5 Step A — flipped from ALWAYS-FIRE to UNLESS-planner-rec-clean-AND-§1-populated.
- Outer CLAUDE.md gate-1.5 P-orch-8 — orchestrator-side skip-condition mirror.
- User memory `feedback_locked_fork_details_appendix.md` — original directive (locked-fork-details section before approval; v32 satisfies via iter-1-populated bodies).

**Rationale (cross-cycle evidence)**:

The 7-of-7 i-phi cycle plan-iteration-count pattern (CH-05/CH-06/CH-08/CH-16a/CH-16b/CH-07a/CH-10/CH-11a/CH-17) cited at outer CLAUDE.md `Plan-iteration-count cross-cycle pattern` showed that iter-2 re-spawn fires reliably for USER-DIVERGENT absorption (sub-classes (i)–(iv)) AND for v22 P13 / v23 P-plan-3 appendix-only-absorption. For planner-rec-clean cycles, the iter-2 re-spawn was almost-entirely the locked-fork-details appendix authoring — a cost the iter-1-populated approach eliminates. The chunk-archive-plan hard-assertion is the regression defense.

**Project-agnostic note**: P-plan-1-v32 applies uniformly to `project=baby-phi` AND `project=i-phi` AND any future project. No project-specific path literals in the rule directive.

### P-plan-2-v32 — Post-fork-lock §3.B per-file LOC cap re-derivation refinement (added 2026-05-25 per CH-17-i-phi retro `e764aeca` proposal #5 MEDIUM)
...
### P-plan-1-v34 — Cross-transport asymmetry stress-test at §3 (HIGH; closes CC-04 H4 refined hypothesis-axis gap)

When a chunk ships ≥ 2 transport variants of the same feature (UDS + TCP, sync + async, HTTP/1 + HTTP/2, REST + WebSocket, etc.), enumerate the asymmetry axes in plan §3 + flag candidate diagnostic tests for the hazard surface. Required asymmetry axes to evaluate per fork-set:

- **Eager-await vs lazy-poll**: does each transport's underlying library `await` the underlying I/O eagerly (e.g., `hyper-util` legacy client) or lazily on first `next().await` (e.g., `reqwest-eventsource`)? If asymmetric, surface as a §3 stress-test row + flag in §7 P-DIAG candidate phase.
- **Connection-pool semantics**: does each transport reuse connections by default? If asymmetric, flag pool-exhaustion / dead-connection hazards.
- **Keepalive defaults**: does each transport have explicit keepalive (interval + timeout)? If asymmetric, flag idle-connection-drop hazards.
- **Buffering semantics**: line-buffered / block-buffered / unbuffered? Stream-flush discipline?
- **Error-propagation shape**: do transport-level errors map to the same client-facing error variants?

For each asymmetric axis, EITHER include a planning-time mitigation in plan §1 lock body OR file a planning-time diagnostic test in plan §8 stress-test cardinality.

**Empirical 1-cycle precedent (CC-03 → CC-04)**: CC-03 F1.c shipped UDS + TCP without enumerating the eager-await vs lazy-poll asymmetry → D-CC03-FOLLOWUP-07 Severity HIGH production hazard → CC-04 dedicated debug chunk (~870 LOC, 1-day wall-clock). H4 refined hypothesis: `reqwest-eventsource` lazy-poll vs `hyper-util` eager-await race. Fix at `src/client/mod.rs:438-551`. Applying P-plan-1-v34 at CC-03 plan-draft would have surfaced the H4 hypothesis at planning-tier; ZERO production hazard. Cross-references: ADR-0025 §D25.1-A, ADR-0003 §D3.2 (UDS-primary policy).

### P-plan-2-v34 — §4 drift-closure column distinction this-cycle vs prior-cycle-ratified (MEDIUM; closes CC-01 G-5 + CC-02 G-9 plan-narrative observations)

Plan §4 drift-closure table column conventions (amends prior "Closes at chunk-close" canonical wording to distinguish two routings):

- Column **`Status flip at this cycle`**: list drifts whose Status row transitions at THIS cycle's P-SEAL (e.g., `discovered → remediated`).
- Column **`Prior-cycle closure ratified at this cycle`**: list drifts already closed at a prior cycle whose plan §1 lock-body cites the closure as a precondition for THIS cycle's deliverables. No state transition at this cycle.

Per chunk-implementer P-SEAL self-check: when authoring §4, BEFORE commit `grep -nE 'Status: (closed|remediated|partially-remediated)' <drift-files>` and confirm Status flip date matches THIS cycle's date. If date is PRIOR-cycle, route to "Prior-cycle closure ratified" column with prior-cycle citation.

**Empirical 2-cycle pattern (CC-01 G-5 + CC-02 G-9)**: CC-01 plan §4 cited D-CH09-FOLLOWUP-01 + D-CH07a-FOLLOWUP-01 closures with imprecise "Closes at chunk-close" wording (both drifts actually closed at prior CH-RT-01 cycle `b0f988ff` 2026-05-28; CC-01 only ratifies). CC-02 plan §3.A cited "PARTIAL at CH-CC-01" annotation as residing in bootstrap.md; actual location was ADR-0020 §D20.5-A. Distinguishing this-cycle from prior-cycle ratification eliminates the cite-imprecision class. Companion to per-chunk-planning-template.md §4 column header amendment.

### P-plan-3-v34 — §3 cascade-band production-tier vs test-tier distinction (MEDIUM; closes CC-03 G-13 Audit A iter-1 claim 11 PASS-with-caveat)

Cascade-band predictions MUST distinguish `production-tier cascade ≤ N` from `test-tier expected-assertion contribution ~M`. Plan §3 cascade-grep pre-flight runs two greps (refines v14 R1 cascade-grep pre-flight):

```bash
grep -rn '<pattern>' src/ | wc -l         # production-tier count (band-constrained)
grep -rn '<pattern>' tests/ | wc -l       # test-tier expected-assertion contribution (upper-bound assertion, NOT band-constrained)
```

Plan-narrative band declarations use only the production-tier number; auditor verification cites both numbers separately.
...
§1 Locked fork details (per v32 P-plan-1-v32): <populated for all forks at iter-1 | n/a (no forks)>
  - chunk-template-validate-locked-appendix skill: PASS | FAIL <reason>
...
### v37 — Four-update bundle from joint-retro `06f9e112..53c7780f` (MA-cluster batch 2, 2026-08-18)

#### P-plan-1-v37 — §3.B LOC-cap: split into logic-cap + doc/test allowance (#3; 3rd occurrence CH-05/MA-04/MA-08)
When predicting a §3.B per-file LOC cap, derive TWO numbers: a **logic cap** (the functional body — the match/loop/struct) AND an explicit **doc-comment + inline-test allowance** (a codebase that writes 18-20-line sibling field doc-comments + co-located Tier-A/B `#[cfg(test)]` blocks routinely adds 2-4× the logic LOC in docs+tests). Surface both in the §3.B cap body (`<logic> + <doc/test allowance> = <cap>`). Empirical: MA-08 `registry.rs` shipped ~30 logic LOC + ~50 doc-comment + ~103 inline-test = 215 (vs a logic-only cap of 75 → a spurious 2.9× "overrun" that was pure doc+test density). CH-05 D-1 (parser.rs) + MA-04 D-1 + MA-08 D-2 are the 3-occurrence precedent. Pairs with chunk-implementer P-impl-1-v28 (the implementer-side pause-downgrade).

#### P-plan-2-v37 — §8 per-agent test-band MUST include a LIVE-overlay (no-restart) assertion (#7; MA-06 gap)
For any per-agent isolation / create-payload chunk (the field-substitution class), the §8 test band MUST allocate a **live-overlay assertion** — a runtime-created (`create_agent`/overlay) agent enforcing its per-agent value WITHOUT a daemon restart — NOT only a config-authored persist+reload assertion. MA-06's green +9 deterministic suite tested only reload; the runtime-overlay path silently fell back to the daemon-wide default until restart (the live close-gate caught it). Invariant to encode in §6/§8: **any per-agent field wired into `from_config` MUST also be wired into `insert_runtime_entry`, and the suite MUST assert it live.** See chunk-p0-investigator "Per-agent field-substitution playbook".

#### P-plan-3-v37 — §3.C lists ONLY docs that will change; codify the `/agents`-wire doc-home (#8; MA-07 D-2)
§3.C (user-facing docs) MUST list only docs the chunk will actually amend — an over-listed §3.C creates a false-FAIL surface for auditors + a widened-sweep target that never fires. Specifically: the i-phi `/agents` wire contract lives in `design/daemon.md` + `specs/agent.md`, **NOT** `specs/interfaces.md` / `specs/api.md` — do not list the latter for an `/agents`-route chunk. MA-07 D-2 (§3.C over-listed interfaces.md/api.md; neither changed) is the recurring MA-01b/01c precedent.

#### P-plan-4-v37 — Weight security-adjacent forks by the config's ROLE, not a flat "security-adjacent" signal (#6; MA-08 F2.b non-fire)
When framing a security-adjacent fork's default-recommendation, weight it by **what the config GOVERNS**: **trusted operator-authored config** (a `permissions_path` / `system_prompt_path` / memory-root / skills-dirs — same trust tier the operator already controls) → planner-rec (sibling-consistent, not max-defensive) is the likely user lock; an **untrusted-input boundary** (network-facing, LLM-tool-arg, external request) → the more-defensive option is the likely divergence. MA-08 F2.b (cwd-verbatim path resolution) did NOT diverge despite being flagged an axis-(c) security-adjacent watch, because `permissions_path` is trusted operator-config; the planner over-predicted divergence toward fail-loud (mirroring the CC-28 F2 skills-format over-prediction). Extends v33 P-plan-3-v33 axis-(c) anticipation + the CLAUDE.md domain-conditional refinement.
